...
pub fn build(b: *std.Build) !void {
    ...
    const kw_action = b.addModule("kw-action", .{
        .root_source_file = b.path("src/action.zig"),
        .target = target,
        .optimize = optimize,
    });

    kw_action.addOptions("build_config", o);

    const tests = b.addTest(.{
        .root_module = kw_action,
        .use_llvm = true,
    });
    ...
    // Imports:
    // 1st Party:
    kw_action.addImport("kw-core", kw_core);
    kw_action.addImport("klib", klib);
}
