...
pub fn build(b: *std.Build) !void {
    ...
    const kw_cache = b.addModule("kw-cache", .{
        .root_source_file = b.path("src/cache.zig"),
        .target = target,
        .optimize = optimize,
    });

    kw_cache.addOptions("build_config", o);

    const tests = b.addTest(.{
        .root_module = kw_cache,
        .use_llvm = true,
    });
    ...
    // Imports:
    // 1st Party:
    kw_cache.addImport("kw-core", kw_core);
    kw_cache.addImport("klib", klib);
}
