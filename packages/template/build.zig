...
pub fn build(b: *std.Build) !void {
    ...
    const kw_placeholder = b.addModule("kw-placeholder", .{
        .root_source_file = b.path("src/driver.zig"),
        .target = target,
        .optimize = optimize,
    });

    kw_placeholder.addOptions("build_config", o);

    const tests = b.addTest(.{
        .root_module = kw_placeholder,
        .use_llvm = true,
    });
    ...
    // Imports:
    // 1st Party:
    kw_placeholder.addImport("kw-core", kw_core);
    kw_placeholder.addImport("klib", klib);
}
